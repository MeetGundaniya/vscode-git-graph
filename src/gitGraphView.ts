...
export class GitGraphView extends Disposable {
	 ...
	 */
	public static scrollToCommit(hash: string, alwaysCenterCommit: boolean, flash: boolean = false, persistently: boolean = false) {
		if (GitGraphView.currentPanel) {
			GitGraphView.currentPanel.respondScrollToCommit(hash, alwaysCenterCommit, flash, persistently);
		}
	}
	 ...
	 * @param persistently Persistently find the commit even if it is not exists.
	 */
	public respondScrollToCommit(hash: string, alwaysCenterCommit: boolean, flash: boolean = false, persistently: boolean = false) {
		this.sendMessage({
